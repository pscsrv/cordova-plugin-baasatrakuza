    ...
    describe('ユーザー関連API', function() {
        describe('新規登録する場合', function() {
            ...
            it('正しくユーザー情報が作成されること', function(done) {
                var loginId = helper.dateFormat.format(new Date(), 'yyyyMMddhhmmss');
                var user_name = "コルドバ太郎【新規登録】" + new Date();
                var user = {
                    user_name: user_name,
                    // ログインIDはユニークでないとダメなので日付で対応
                    login_id : loginId,
                    user_first_name : "太郎",
                    user_last_name : "コルドバ",
                ...
                RKZClient.registUser(user,
                    function(user) {
                        ...
                        expect(user.user_id).toMatch(/^21003[0-9]{10}$/);
                        ...
                        expect(user).toEqual(jasmine.objectContaining({"login_id" : loginId}));
                        ...
                        expect(user).toEqual(jasmine.objectContaining({"point" : null}));
                        expect(user.last_update_dte).toMatch(/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\+0900$/);

                        // expect(Object.keys(user.attributes).length).toEqual(24);
                        ...
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"prefecture" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"address1" : null}));
                        ...
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"postal_code" : null}));
                        ...
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"prefecture_name" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"remarks" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"mail_address_1" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"activity_area_cd_name" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"age_config_name" : null}));
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"address2" : null}));
                        ...
                        // expect(user.attributes).toEqual(jasmine.objectContaining({"state_cd_name" : null}));

                        // 以降のテストで利用するユーザー情報を保持する。
                        helper.userAccessToken = user.user_access_token;
                        _user_no = user.user_no;
                        _login_id = user.login_id;
                        done();
                    }, function(error) {
