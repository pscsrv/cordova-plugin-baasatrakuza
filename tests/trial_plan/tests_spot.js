            ...
            it('検索条件を指定したら、正しく検索されること', function(done) {
                ...
                RKZClient.getSpotList(searchConditions, sortConditions,
                    function(spots) {
                        ...
                        expect(spots[0]).toEqual(jasmine.objectContaining({short_name: "A1"}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({sort_no: 3}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({beacon_range_for_android: -80}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({beacon_range_for_iphone: -80}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({latitude: '34.602446'}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({longitude: '133.765669'}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({pixel_position_x: "10"}));
                        expect(spots[0]).toEqual(jasmine.objectContaining({pixel_position_y: "20"}));
